...
export const validateCaptcha = async (req: Request, res: Response): Promise<void> => {
  const { apiKey, challengeToken, captchaResponse, solution } = req.body;
  const isValid = await captchaService.validateCaptcha(req.app.locals.connection, apiKey, challengeToken, captchaResponse, solution);
  if (isValid) {
